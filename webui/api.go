	...
	return func(c *gin.Context) {
		if strings.HasPrefix(c.Request.URL.Path, "/webui/api") {
			...
			if c.Param("filepath") == "/api/"+appIDStr+"/api" && c.Request.Method == http.MethodPost {
				...
				case "get_guild_list":
					// 处理获取群组列表的请求
					handleGetGuildList(c, s)
				case "get_channel_list":
					// 处理获取频道列表的请求
					handleGetChannelList(c, s)
				case "send_guild_channel_message":
					// 调用处理发送消息的函数
					handleSendGuildChannelMessage(c, s)
				default:
					// 处理其他或未知的api名称
...
func handleSendGuildChannelMessage(c *gin.Context, s *discordgo.Session) {
	...
	// 使用提供的channelID和消息内容发送消息
	message, err := s.ChannelMessageSend(req.ID, req.Message)
	if err != nil {
		// 信息发送失败，返回失败原因
...
func handleGetGuildList(c *gin.Context, s *discordgo.Session) {
	// 提取前端发来的 pager 数据
	var pager GuildPager
	...
	// 解析 limit 参数
	limit, err := strconv.Atoi(pager.Limit)
	if err != nil || limit < 1 || limit > 100 {
		// 如果 limit 参数无效，设置默认值为 100
		limit = 100
	}

	// 调用 discordgo 的 UserGuilds 方法
	guilds, err := s.UserGuilds(limit, pager.Before, pager.After)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	// 将 discordgo 数据转换为前端需要的格式
...
func handleGetChannelList(c *gin.Context, s *discordgo.Session) {
	...
	guildChannels, err := s.GuildChannels(pager.After)
	if err != nil {
		// 错误处理
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	...
	// 将后端数据转换为前端需要的格式
	channelList := make([]map[string]interface{}, len(guildChannels))
