...
func (p *Processors) ProcessChannelDirectMessage(data *discordgo.MessageCreate, se *discordgo.Session) error {
	...
	if !p.Settings.GlobalPrivateToChannel {
		...
		//转换at
		messageText := handlers.RevertTransformedText(data, "guild_private", se, userid64)
		if messageText == "" {
			mylog.Printf("信息被自定义黑白名单拦截")
			return nil
		}
		//框架内指令
		p.HandleFrameworkCommand(messageText, data, "guild_private", se)
		...
		if !p.Settings.GlobalChannelToGroup {
			...
			s := discordgo.GetGlobalS()
			//转换at
			messageText := handlers.RevertTransformedText(data, "guild_private", se, 10000) //todo 这里未转换
			if messageText == "" {
				mylog.Printf("信息被自定义黑白名单拦截")
				return nil
			}
